...
def convolve(image, kernel):
  ...
  output = np.zeros((image.shape[0] - kernel.shape[0] + 1 , image.shape[1] - kernel.shape[1] + 1) , dtype = image.dtype)
  
  kernel_reversed = kernel[::-1,::-1]

  for i in range(0, output.shape[0]):
    for j in range(0, output.shape[1]):
      image_patch = image[i:i+(kernel.shape[0]), j:j+(kernel.shape[1])]
  
      output[i,j] = ((kernel_reversed * image_patch).sum()).clip(min=0, max=255)
