...
def blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask):
  ...
  alpha = gauss_pyr_mask

  blended_pyr = [[] for i in range(len(gauss_pyr_mask))]
  
  for i in range (0, len(gauss_pyr_mask)):
    blended_pyr[i] = alpha[i]*lapl_pyr_white[i]+(1-alpha[i])*lapl_pyr_black[i]
...
def collapse(lapl_pyr):
  ...
  numLayers = len(lapl_pyr)
  
  for i in range(numLayers-1, 0, -1):
    expanded = part0.expand(lapl_pyr[i])
    baseRows = lapl_pyr[i-1].shape[0]
    baseCols = lapl_pyr[i-1].shape[1]

    if (expanded.shape[0] > baseRows):
      expanded = expanded[:baseRows,:]

    if (expanded.shape[1] > baseCols):
      expanded = expanded[:,:baseCols]

    output = expanded + lapl_pyr[i-1]
