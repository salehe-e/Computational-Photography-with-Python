...
def reduce(image):
  ...
  out = np.zeros((np.ceil(image.shape[0]/2),np.ceil(image.shape[1]/2)), dtype = float)

  filtered_image = scipy.signal.convolve(image, kernel, 'same')

  out = filtered_image[::2,::2]
...
def expand(image):
  ...
  I = np.zeros((image.shape[0]*2,image.shape[1]*2), dtype = float)

  I[::2,::2] = image

  out = 4*scipy.signal.convolve(I, kernel, 'same')
