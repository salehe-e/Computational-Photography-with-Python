...
def interlace(evens, odds):
  ...
  outimg = np.zeros((2*evens.shape[0],evens.shape[1],evens.shape[2]), dtype=evens.dtype)

  outimg[0:outimg.shape[0]:2, :, :] = evens[np.arange(0,evens.shape[0],1), :, :]
  outimg[1:outimg.shape[0]:2, :, :] = odds[np.arange(0,odds.shape[0],1), :, :]
